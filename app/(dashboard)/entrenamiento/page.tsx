...
type PlanDay = {
  ...
  workout_plan_exercises: {
    ...
    order_index: number; notes: string | null; duration_seconds: number | null; phase: "warmup" | "main" | "cooldown"
  ...
  if (role === "member") {
    ...
    const [daysRes, sessionsRes, exerciseMaxes] = await Promise.all([
      supabase
        ...
        .select(`id, day_of_week, name, workout_plan_exercises(id, sets, reps, reps_max, rest_seconds, order_index, notes, duration_seconds, phase, exercises(id, name, category, image_url, muscle_groups, is_timed), set_configs:workout_plan_set_configs(id, set_number, reps, reps_max, percent_1rm, duration_seconds, notes))`)
        .eq("plan_id", plan.id)
