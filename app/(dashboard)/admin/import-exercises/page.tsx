    ...
    <div className="space-y-6 max-w-lg">
      <div>
        ...
        <p className="text-sm text-zinc-500 mt-1">
          Importá o exportá el catálogo de ejercicios en CSV.
        </p>
      </div>
      ...
      <p className="text-xs text-zinc-600">
        El archivo de importación debe estar en <code className="text-zinc-500">data/exercises-from-routine.csv</code> en el servidor.
        La exportación descarga todos los ejercicios actuales de la base de datos.
      </p>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-zinc-100">
                {result.upserted} de {result.total} filas importadas
              </p>
              {result.errors.length > 0 && (
          ...
          {result.errors.length > 0 && (
            <ul className="space-y-1">
              {result.errors.map((e, i) => (
                <li key={i} className="text-sm text-red-400 rounded bg-red-950/30 px-3 py-1.5">
                  {e.message}
                </li>
              ))}
            </ul>
