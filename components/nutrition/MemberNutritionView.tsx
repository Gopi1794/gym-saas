...
import type { MealLog } from "@/app/actions/nutrition-tracking"
...
// ── Main component ────────────────────────────────────────────
export default function MemberNutritionView({ plan, mealLogs, waterGlasses, streak, today }: Props) {
  if (!plan) {
    return (
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <div className="flex items-center justify-around">
          <MacroRing label="Calorías" value={Math.round(consumed.calories)} target={plan.target_calories} unit="kcal" color="#FF2222" trackColor="#3f3f46" />
          <MacroRing label="Proteínas" value={Math.round(consumed.protein)} target={plan.target_protein} unit="g" color="#60a5fa" trackColor="#3f3f46" />
          <MacroRing label="Carbos" value={Math.round(consumed.carbs)} target={plan.target_carbs} unit="g" color="#fbbf24" trackColor="#3f3f46" />
          <MacroRing label="Grasas" value={Math.round(consumed.fat)} target={plan.target_fat} unit="g" color="#34d399" trackColor="#3f3f46" />
        </div>
      </div>
