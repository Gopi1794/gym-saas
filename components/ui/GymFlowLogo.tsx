...
type Props = {
  ...
  showTagline?: boolean
  /** Mostrar solo el rayo, sin el wordmark (para espacios angostos) */
  iconOnly?: boolean
}

export function VoltiaLogo({ className, size = 28, textSize = "text-2xl", showTagline = false, iconOnly = false }: Props) {
  const bolt = (
    <svg
      width={size}
      height={size}
  ...
  if (iconOnly) {
    return <span className={cn("inline-flex", className)}>{bolt}</span>
  }
